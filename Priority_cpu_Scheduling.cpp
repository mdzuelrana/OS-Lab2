...
struct Process {
    int pid, arrival, burst, priority, start, finish, waiting, turnaround;
};
...
int main() {
    ...
    while (!p.empty()) {
        ...
        auto highest = std::min_element(ready.begin(), ready.end(), [](Process a, Process b) {
    ...
    std::cout << "PID\tPriority\tWT\tTAT\n";
    for (auto& proc : done)
        std::cout << proc.pid << "\t" << proc.priority << "\t\t" << proc.waiting << "\t" << proc.turnaround << "\n";
